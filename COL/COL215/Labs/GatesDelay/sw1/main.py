...
def output_b(dict, primary_outputs):
    output = []
    not_possible = True
    ...
    if not not_possible:
        for outputs in primary_outputs:
            output.append(f"{outputs} {(dict[outputs]):.0f}")
...
def output_a(dict, primary_outputs):
    output = []
    for outputs in primary_outputs:
        output.append(f"{outputs} {(dict[outputs]):.0f}")
...
def secondary_check(dict_a, ans_dict, primary_inputs, primary_outputs, gates):
    ...
    for a in range(upper_bound, 0, -1):
        for b in range(upper_bound, 0, -1):
...
def main_b():
    ...
    if final_check(dict_a, ans_dict, primary_inputs, primary_outputs, gates):
        output_b(dict_a, primary_inputs)
    else:
