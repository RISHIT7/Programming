...
public:
    ...
    int insertion(int data)
    {
        ...
        else if (data > set[set.size() -1])
        {
            set.push_back(data);
            return set.size();
        }
        else if (data == set[0] || data == set[set.size() -1])
        {
            return set.size();
        }
    ...
    {
        SET *Union_set = new SET(set);
        Union_set->Union(set2);

        SET *Intersection_set = new SET(set);
