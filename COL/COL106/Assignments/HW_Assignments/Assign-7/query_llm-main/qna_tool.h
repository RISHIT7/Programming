...
vector<vector<vector<SymbolTable>>> mainobj; // main data struct
...
struct scored_para
{
    ...
    int page_no;
    int para_no;
    double score; // how accurate the paragraph is on the basis of the input query
};

vector<scored_para> tity(string sentence);


vector<pair<string, double>> scoring(string sentence); // first scoring the wwords and then the paragraph
