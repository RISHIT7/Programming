...
def create_structure(space, width, height):
    BROWN = (139, 69, 19, 100)
    rects = [
        [(600, height-120), (40, 200), BROWN, 50],
        [(900, height-120), (40, 200), BROWN, 50],
        [(750, height-240), (340, 40), BROWN, 55]
    ]
...
def create_ball(space, radius, pos, mass):
    body = pymunk.Body(body_type=pymunk.Body.STATIC)
    body.position = pos
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if not ball:
                    ...
                    ball = create_ball(space, 15, pressed_pos, 10)
                elif pressed_pos:
                    ball.body.body_type = pymunk.Body.DYNAMIC
                    angle = calculate_Angle(*line)
                    force = calculate_distance(*line) * -32
